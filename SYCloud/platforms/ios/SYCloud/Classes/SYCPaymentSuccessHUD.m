...
@implementation SYCPaymentSuccessHUD
{
...
+(SYCPaymentSuccessHUD*)showIn:(UIView*)view{
    [self hideIn:view];
    SYCPaymentSuccessHUD *hud = [[SYCPaymentSuccessHUD alloc] initWithFrame:view.bounds];
    [hud start];
    [view addSubview:hud];
    return hud;
}
...
+(SYCPaymentSuccessHUD *)hideIn:(UIView *)view{
    SYCPaymentSuccessHUD *hud = nil;
    for (SYCPaymentSuccessHUD *subView in view.subviews) {
        if ([subView isKindOfClass:[SYCPaymentSuccessHUD class]]) {
            [subView hide];
